...
/**
 * FAQ accordion for the About page — same content as the homepage
 * FAQ, but its own component so it can have its own #F5F8FF
 * section background independent of the homepage's FAQ.
 */
  ...
  return (
    <section className="about-faq">
      <div className="about-faq__inner">
        <h2 className="about-faq__heading">
          Frequently Asked{" "}
          <span className="about-faq__heading-accent">Questions</span>
        </h2>
        ...
        <div className="about-faq__list">
          {FAQS.map((item, index) => {
            ...
            return (
              <div className="about-faq__card" key={item.question}>
                <button
                  type="button"
                ...
                {isOpen && <p className="about-faq__answer">{item.answer}</p>}
              </div>
            );
          })}
        </div>
